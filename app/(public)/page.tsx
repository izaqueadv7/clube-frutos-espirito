...
export default async function HomePage() {
  ...
  const innerCardClass =
    "rounded-2xl border page-line bg-app-card px-5 py-4 shadow-[0_4px_12px_rgba(0,0,0,0.08)]";
      ...
      <section className="mx-auto grid max-w-7xl gap-8 px-4 py-12 sm:px-6 lg:grid-cols-2 lg:px-8 lg:py-16">
        ...
        <Card className="overflow-hidden p-0">
          ...
          <div className="space-y-4 p-6">
            <div className="rounded-2xl bg-red-50 p-4 dark:bg-app-soft">
            ...
            <div className="rounded-2xl bg-yellow-50 p-4 dark:bg-app-soft">
              <p className="text-sm font-semibold text-amber-700 dark:text-primary">
                Acesso para famílias
              </p>
              <p className="text-sm text-app-secondary">
          ...
          <div className="mt-4 space-y-4">
            {events.map((event) => (
              <div key={event.id} className={innerCardClass}>
                ...
                <p className="mt-1 text-sm text-app-secondary">{event.location}</p>
              </div>
            ))}
          </div>
        </Card>
          ...
          <div className="mt-4 space-y-4">
            {announcements.map((item) => (
              <div key={item.id} className={innerCardClass}>
                <p className="font-semibold text-app-primary">{item.title}</p>
                <p className="mt-1 text-sm text-app-secondary">{item.content}</p>

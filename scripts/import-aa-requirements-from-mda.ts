...
const AREA_URL = "https://mda.wiki.br/Atividades_Agr%C3%ADcolas";
    ...
    if (plainText) {
      const fallbackLines = plainText
        ...
        .filter(
          (line) =>
            line.length > 20 &&
            !/^(especialidade de|area|área|c[oó]digo|n[ií]vel|ano|institui[cç][aã]o de origem|conte[uú]do retirado do site|categorias|p[aá]ginas relacionadas)/i.test(
              line
            )
        );
...
async function findSpecialtyByTitle(title: string) {
  ...
  const relaxed = specialties.find((item) => {
    const db = normalizeName(item.name).replace(/ii/g, "2");
    const src = normalizedTitle.replace(/ii/g, "2");
    return db === src;
  });
...
async function main() {
  console.log("Importando requisitos de AA...");
...
main()
  .catch((error) => {
    console.error("Erro ao importar requisitos de AA:", error);
    process.exit(1);
  })
  .finally(async () => {

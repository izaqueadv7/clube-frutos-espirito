...
async function findSpecialtyByTitle(title: string) {
  ...
  const relaxed = specialties.find((item) => {
    const db = normalizeName(item.name)
      ...
      .replace(/juventil/g, "juvenil")
      .replace(/hist[oó]rias/g, "historias");
...
async function main() {
  console.log("Importando requisitos de AM...");
...
main()
  .catch((error) => {
    console.error("Erro ao importar requisitos de AM:", error);
    process.exit(1);
  })
  .finally(async () => {
